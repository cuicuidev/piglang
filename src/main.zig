const std = @import("std");

pub fn main() !void {
    const source_code =
        \\pub fn main() !void:
        \\    const x: u8 = 100
        \\    y: f32 = 69.0
        \\    
        \\    y += 420.0
        \\    y = y + @asFloat(f32, x)
        \\    
        \\    @println("Result: {d}", .{y})
        \\
    ;

    const stdout = std.io.getStdOut().writer();
    try stdout.writeAll(source_code);

    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();

    var allocator = gpa.allocator();

    var lexer = Lexer.init(&allocator, source_code);
    defer lexer.deinit();

    try lexer.tokenize();

    for (lexer.tokens.items) |token| {
        try stdout.print("Token{{ .value = \"", .{});
        for (token.value) |char| {
            if (char == '\n') {
                try stdout.writeAll("\\n");
            } else {
                try stdout.print("{c}", .{char});
            }
        }
        try stdout.print("\", ._type = {} }}\n", .{token._type});
    }
}

const TokenType = enum { IDENTIFIER, BINARY_OPERATOR, SEPARATOR, INT_LITERAL, FLOAT_LITERAL, CHAR_LITERAL, STRING_LITERAL, BOOLEAN_LITERAL, NULL_LITERAL, OPEN_PAREN, CLOSE_PAREN, OPEN_BRACKET, CLOSE_BRACKET, OPEN_CURLY_BRACE, CLOSE_CURLY_BRACE, WHITE_SPACE, LINE_BREAK };

const Token = struct {
    value: []const u8,
    _type: TokenType,

    const Self = @This();

    pub fn init(value: []const u8, _type: TokenType) Self {
        return .{ .value = value, ._type = _type };
    }
};

const LexerState = enum { NEW_TOKEN, COMPLETE_TOKEN, IDENTIFIER, INT_LITERAL, FLOAT_LITERAL, CHAR_LITERAL, STRING_LITERAL, OPERATOR, OPEN_PAREN, CLOSE_PAREN, OPEN_BRACKET, CLOSE_BRACKET, OPEN_CURLY_BRACE, CLOSE_CURLY_BRACE, WHITE_SPACE, LINE_BREAK };

const LexerErr = error{UnrecognizedCharacter};

const StringSlice = struct { slice: []const u8, start_idx: usize };

const Lexer = struct {
    source: []const u8,
    pos: usize,
    max_idx: usize,
    state: LexerState,
    byte_buffer: StringSlice,
    tokens: std.ArrayList(Token),
    allocator: *std.mem.Allocator,

    const Self = @This();

    pub fn init(allocator: *std.mem.Allocator, source: []const u8) Self {
        const tokens = std.ArrayList(Token).init(allocator.*);
        const byte_buffer: StringSlice = StringSlice{ .slice = source[0..0], .start_idx = 0 };
        return .{ .source = source, .pos = 0, .max_idx = source.len, .state = LexerState.NEW_TOKEN, .byte_buffer = byte_buffer, .tokens = tokens, .allocator = allocator };
    }

    pub fn deinit(self: *Self) void {
        self.tokens.deinit();
    }

    pub fn tokenize(self: *Self) !void {
        while (self.pos < self.max_idx) {
            try self._next();
        }
    }

    fn _next(self: *Self) !void {
        const char = self.source[self.pos];
        var next_char: ?u8 = undefined;
        if (self.pos < self.source.len - 1) {
            next_char = self.source[self.pos + 1];
        } else {
            next_char = null;
        }
        switch (self.state) {
            LexerState.NEW_TOKEN => {
                if (isAsciiLetterOrUnderscore(char)) {
                    self.state = LexerState.IDENTIFIER;
                    return;
                }

                if (isDigit(char)) {
                    self.state = LexerState.INT_LITERAL;
                    return;
                }

                if (char == '\'') {
                    self.state = LexerState.CHAR_LITERAL;
                    return;
                }

                if (char == '"') {
                    self.state = LexerState.STRING_LITERAL;
                    return;
                }

                if (isSymbol(char)) {
                    self.state = LexerState.OPERATOR;
                    return;
                }

                if (char == '(') {
                    self.state = LexerState.OPEN_PAREN;
                    return;
                }

                if (char == ')') {
                    self.state = LexerState.CLOSE_PAREN;
                    return;
                }

                if (char == '[') {
                    self.state = LexerState.OPEN_BRACKET;
                    return;
                }

                if (char == ']') {
                    self.state = LexerState.CLOSE_BRACKET;
                    return;
                }

                if (char == '{') {
                    self.state = LexerState.OPEN_CURLY_BRACE;
                    return;
                }

                if (char == '}') {
                    self.state = LexerState.CLOSE_CURLY_BRACE;
                    return;
                }

                if (isWhiteSpace(char)) {
                    self.state = LexerState.WHITE_SPACE;
                    return;
                }

                if (char == '\n') {
                    self.state = LexerState.LINE_BREAK;
                    return;
                }
            },
            LexerState.COMPLETE_TOKEN => {
                self.byte_buffer = StringSlice{ .slice = self.source[self.pos..self.pos], .start_idx = self.pos };
                self.state = LexerState.NEW_TOKEN;
            },
            LexerState.IDENTIFIER => {
                self.pos += 1;
                self.byte_buffer = StringSlice{ .slice = self.source[self.byte_buffer.start_idx..self.pos], .start_idx = self.byte_buffer.start_idx };

                if (next_char) |c| {
                    if (!isAsciiLetterOrUnderscore(c) and !isDigit(c)) {
                        const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.IDENTIFIER };
                        try self.tokens.append(token);
                        self.state = LexerState.COMPLETE_TOKEN;
                    }
                } else {
                    const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.IDENTIFIER };
                    try self.tokens.append(token);
                    self.state = LexerState.COMPLETE_TOKEN;
                }
            },
            LexerState.INT_LITERAL => {
                self.pos += 1;
                self.byte_buffer = StringSlice{ .slice = self.source[self.byte_buffer.start_idx..self.pos], .start_idx = self.byte_buffer.start_idx };

                if (next_char) |c| {
                    if (isWhiteSpace(c) or isSymbol(c)) {
                        const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.INT_LITERAL };
                        try self.tokens.append(token);
                        self.state = LexerState.COMPLETE_TOKEN;
                    }
                    if (c == '.' or c == 'e' or c == 'E') {
                        self.state = LexerState.FLOAT_LITERAL;
                    }
                } else {
                    const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.INT_LITERAL };
                    try self.tokens.append(token);
                    self.state = LexerState.COMPLETE_TOKEN;
                }
            },
            LexerState.FLOAT_LITERAL => {
                self.pos += 1;
                self.byte_buffer = StringSlice{ .slice = self.source[self.byte_buffer.start_idx..self.pos], .start_idx = self.byte_buffer.start_idx };

                if (next_char) |c| {
                    if (isWhiteSpace(c) or isSymbol(c)) {
                        const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.FLOAT_LITERAL };
                        try self.tokens.append(token);
                        self.state = LexerState.COMPLETE_TOKEN;
                    }
                } else {
                    const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.FLOAT_LITERAL };
                    try self.tokens.append(token);
                    self.state = LexerState.COMPLETE_TOKEN;
                }
            },
            LexerState.CHAR_LITERAL => {
                self.pos += 1;
                self.byte_buffer = StringSlice{ .slice = self.source[self.byte_buffer.start_idx..self.pos], .start_idx = self.byte_buffer.start_idx };

                const c = next_char.?;

                if (char != '\\') {
                    if (c == '\'') {
                        self.pos += 1;
                        self.byte_buffer = StringSlice{ .slice = self.source[self.byte_buffer.start_idx..self.pos], .start_idx = self.byte_buffer.start_idx };

                        const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.CHAR_LITERAL };
                        try self.tokens.append(token);
                        self.state = LexerState.COMPLETE_TOKEN;
                    }
                }
            },
            LexerState.STRING_LITERAL => {
                self.pos += 1;
                self.byte_buffer = StringSlice{ .slice = self.source[self.byte_buffer.start_idx..self.pos], .start_idx = self.byte_buffer.start_idx };

                const c = next_char.?;

                if (char != '\\') {
                    if (c == '"') {
                        self.pos += 1;
                        self.byte_buffer = StringSlice{ .slice = self.source[self.byte_buffer.start_idx..self.pos], .start_idx = self.byte_buffer.start_idx };

                        const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.STRING_LITERAL };
                        try self.tokens.append(token);
                        self.state = LexerState.COMPLETE_TOKEN;
                    }
                }
            },
            LexerState.OPERATOR => {
                self.pos += 1;
                self.byte_buffer = StringSlice{ .slice = self.source[self.byte_buffer.start_idx..self.pos], .start_idx = self.byte_buffer.start_idx };

                if (next_char) |c| {
                    if (!isSymbol(c)) {
                        const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.BINARY_OPERATOR };
                        try self.tokens.append(token);
                        self.state = LexerState.COMPLETE_TOKEN;
                    }
                } else {
                    const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.BINARY_OPERATOR };
                    try self.tokens.append(token);
                    self.state = LexerState.COMPLETE_TOKEN;
                }
            },
            LexerState.OPEN_PAREN => {
                self.pos += 1;
                self.byte_buffer = StringSlice{ .slice = self.source[self.byte_buffer.start_idx..self.pos], .start_idx = self.byte_buffer.start_idx };
                const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.OPEN_PAREN };
                try self.tokens.append(token);
                self.state = LexerState.COMPLETE_TOKEN;
            },
            LexerState.CLOSE_PAREN => {
                self.pos += 1;
                self.byte_buffer = StringSlice{ .slice = self.source[self.byte_buffer.start_idx..self.pos], .start_idx = self.byte_buffer.start_idx };
                const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.CLOSE_PAREN };
                try self.tokens.append(token);
                self.state = LexerState.COMPLETE_TOKEN;
            },
            LexerState.OPEN_BRACKET => {
                self.pos += 1;
                self.byte_buffer = StringSlice{ .slice = self.source[self.byte_buffer.start_idx..self.pos], .start_idx = self.byte_buffer.start_idx };
                const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.OPEN_BRACKET };
                try self.tokens.append(token);
                self.state = LexerState.COMPLETE_TOKEN;
            },
            LexerState.CLOSE_BRACKET => {
                self.pos += 1;
                self.byte_buffer = StringSlice{ .slice = self.source[self.byte_buffer.start_idx..self.pos], .start_idx = self.byte_buffer.start_idx };
                const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.CLOSE_BRACKET };
                try self.tokens.append(token);
                self.state = LexerState.COMPLETE_TOKEN;
            },
            LexerState.OPEN_CURLY_BRACE => {
                self.pos += 1;
                self.byte_buffer = StringSlice{ .slice = self.source[self.byte_buffer.start_idx..self.pos], .start_idx = self.byte_buffer.start_idx };
                const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.OPEN_CURLY_BRACE };
                try self.tokens.append(token);
                self.state = LexerState.COMPLETE_TOKEN;
            },
            LexerState.CLOSE_CURLY_BRACE => {
                self.pos += 1;
                self.byte_buffer = StringSlice{ .slice = self.source[self.byte_buffer.start_idx..self.pos], .start_idx = self.byte_buffer.start_idx };
                const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.CLOSE_CURLY_BRACE };
                try self.tokens.append(token);
                self.state = LexerState.COMPLETE_TOKEN;
            },
            LexerState.WHITE_SPACE => {
                self.pos += 1;
                self.byte_buffer = StringSlice{ .slice = self.source[self.byte_buffer.start_idx..self.pos], .start_idx = self.byte_buffer.start_idx };

                if (next_char) |c| {
                    if (!isWhiteSpace(c)) {
                        const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.WHITE_SPACE };
                        try self.tokens.append(token);
                        self.state = LexerState.COMPLETE_TOKEN;
                    }
                } else {
                    const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.WHITE_SPACE };
                    try self.tokens.append(token);
                    self.state = LexerState.COMPLETE_TOKEN;
                }
            },
            LexerState.LINE_BREAK => {
                self.pos += 1;
                self.byte_buffer = StringSlice{ .slice = self.source[self.byte_buffer.start_idx..self.pos], .start_idx = self.byte_buffer.start_idx };
                const token = Token{ .value = self.byte_buffer.slice, ._type = TokenType.LINE_BREAK };
                try self.tokens.append(token);
                self.state = LexerState.COMPLETE_TOKEN;
            },
        }
    }
};

fn isAsciiLetterOrUnderscore(char: u8) bool {
    const valid_chars = "qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM_";
    for (valid_chars) |c| {
        if (char == c) return true else continue;
    }
    return false;
}

fn isDigit(char: u8) bool {
    const valid_chars = "0123456789";
    for (valid_chars) |c| {
        if (char == c) return true else continue;
    }
    return false;
}

fn isSymbol(char: u8) bool {
    const valid_chars = "+-*/<>=!?@:,.";
    for (valid_chars) |c| {
        if (char == c) return true else continue;
    }
    return false;
}

fn isWhiteSpace(char: u8) bool {
    const valid_chars = " \t\r";
    for (valid_chars) |c| {
        if (char == c) return true else continue;
    }
    return false;
}
